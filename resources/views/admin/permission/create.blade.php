...
@section('content')
    ...
    <div class="card">
        <form method="POST" action="{{ route('permission.store') }}" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-12">
                                ...
                                <tr>
                                    <td>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input myClick" type="checkbox" value="{{$items->id}}" name="permission_id[]">
                                    ...
                                    <td>
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input myCheck{{$item->sub_category}}" type="checkbox" value="{{$item->id}}" name="permission_id[]" >
                                            <label class="form-check-label" >
...
@section('custom-script')
    <script>
        $(document).ready(function(){
            $('.myClick').on('change', function(){ // on change of state
                let id = $(this).val();
   if(this.checked) // if changed state is "CHECKED"
    {
        $(`.myCheck${id}`).attr('checked', true);
    }else
    {
        $(`.myCheck${id}`).attr('checked', false);
    }
})
});
    </script>
