    ...
    <div class="page-header page-header-light">
        <div class="page-header-content header-elements-lg-inline">
            <div class="page-title d-flex">
                <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Lab Test Report</span></h4>
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Lab Test Report</span>
                </div>
            ...
            <table class="table table-bordered table-hover datatable-colvis-basic">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$item->service->service_name}}
                            <ul>
                                @foreach ($item->tests as $tst)
                                    <li>{{$tst->test->tests}}</li>                                
                                @endforeach
                            </ul>
                        </td>
                        <td>{{$item->date}}</td>
                                    ...
                                    <div class="modal fade" id="assignModalCenter_{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="assignModalCenterTitle" aria-hidden="true">
                                        <div class="modal-dialog modal-sm  modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <form action="{{route('servicebookings.assignlab',$item->id)}}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                ...
                                @foreach ($report as $key => $value)
                                    @if($key == $item->id)
                                        @if($value == 0)
                                            @can('Lab Technician')
                                            <td>
                                                <a href="{{route('servicebookings.createReport',$item->id)}}" class="btn btn-primary">Upload Test Report</a>
                                            </td>
                                            @endcan
                                        @else
                                        <td>
                                            <a href="{{route('servicebookings.viewReport',$item->id)}}" class="btn btn-success">View Test Report</a>
                                        </td>
                                        @endif
                                    @endif                            
                                @endforeach     
