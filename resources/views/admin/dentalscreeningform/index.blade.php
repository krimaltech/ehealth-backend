        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Dental Screening Form</span>
                </div>
...
@section('content')
    <div class="card">
        <div class="card-header border-bottom">
            <form action="{{ route('dental-screening-form.index') }}" method="GET">
                @csrf
                ...
                <table class="table table-bordered table-hover datatable-colvis-basic">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                @if ($screening->dentist == null)
                                <a href="{{ route('dental-screening-form.create',$screening->id) }}" class="btn btn-primary"><i class="icon-plus-circle2"></i></a>
                                <a href="{{ route('dental-screening-form.show', $screening->id) }}" class="btn btn-primary" target="_blank"><i
                                    class="icon-eye"></i> </a>
                                @elseif ($screening->dentist != null)
                                <a href="{{ route('dental-screening-form.edit',$screening->dentist->id) }}" class="btn btn-primary"><i class="icon-pen"></i></a>
                                <a href="{{ route('dental-screening-form.show', $screening->id) }}" class="btn btn-primary" target="_blank"><i
                                    class="icon-eye"></i> </a>
                                @else
                                <a href="{{ route('dental-screening-form.show', $screening->id) }}" class="btn btn-primary" target="_blank"><i
                    ...
                    <tbody>
                        @foreach ($screening as $item)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('dental-screening-form.show', $item->id) }}" target="_blank"
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, option) {
                            $('#users_select').append(
                                `<option value="${option.campaign_user_id}" ${option.dentist !== null ? `title="${option.campaign_user_id}"` : ''}>${option.campaignuser.name} (${option.campaignuser.phone})</option>`
                                );
                        });
                    }
                })
