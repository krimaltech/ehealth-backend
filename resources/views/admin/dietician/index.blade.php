    ...
    <div class="page-header page-header-light">
        <div class="page-header-content header-elements-lg-inline">
            <div class="page-title d-flex">
                <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Dietician Screening Form</span></h4>
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-lg-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">Dietician Screening Form</span>
                </div>
...
@section('content')
    <div class="card">
        <div class="card-header border-bottom">
            {{-- <a href="{{ route('dietician.create') }}" type="button" class="btn btn-primary">
                Add Dietician Screening
            </a> --}}
            <form action="{{ route('dietician.index')}}" method="GET">
                    ...
                    @if(Request::get('campaign') && Request::get('users')) 
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="{{route('dietician.edit',$screening->dietician->id)}}" class="btn btn-primary"><i class="icon-pen2"></i></a>
                                @else
                                <a href="{{route('dietician.create',$screening->id)}}" class="btn btn-primary"><i class="icon-plus-circle2"></i></a>
                                @endif
                                <a href="{{route('dietician.show',$screening->id)}}" class="btn btn-primary" target="_blank"><i class="icon-eye2"></i></a>
                            </td>
                        </tr>
                    @else
                        @foreach ($screening as $item)
                            <tr>
                                ...
                                <td>
                                    {{-- @if($item->dietician)
                                    <a href="{{route('dietician.edit',$item->dietician->id)}}" class="btn btn-primary"><i class="icon-pen2"></i></a>
                                    @else
                                    <a href="{{route('dietician.create',$item->id)}}" class="btn btn-primary"><i class="icon-plus-circle2"></i></a>
                                    @endif --}}
                                    <a href="{{route('dietician.show',$item->id)}}" class="btn btn-primary" target="_blank"><i class="icon-eye2"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    @endif
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, option) {
                            $('#users_select').append(
                                `<option value="${option.campaign_user_id}" ${option.dietician !== null ? `title="${option.campaign_user_id}"` : ''}>${option.campaignuser.name} (${option.campaignuser.phone})</option>`
                                );
                        });
                    }
                })
