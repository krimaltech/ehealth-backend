    ...
    <div class="page-header page-header-light">
        <div class="page-header-content header-elements-lg-inline">
            <div class="page-title d-flex">
                <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Drivers</span></h4>
                        ...
                        @foreach ($drivers as $item)
                            <tr>
                                ...
                                @if (Helper::role_count($item->driver_id) == 2)
                                    <td><span>Driver Role Assigned</span></td>
                                @else
                                    <td>
                                        @if ($item->agreement_file == null)
                                            <span>After Agreement</span>
                                        @elseif ($item->agreement_file != null)
                                            <a data-id="{{ $item->driver_id }}" data-role="{{ 9 }}"
                                                class="btn btn-primary deleteBtn">Role assign</a>
                                        @endif
                                    </td>
                                @endif
                                <td>
                                    <a href="{{ route('users.drivershow', $item->id) }}" class="btn btn-primary"><i
                                            class="icon-eye"></i></a>
                                </td>
                            </tr>
                        @endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.deleteBtn').click(function(e) {
                ...
                swal({
                        title: "Are you sure?",
                        text: "You want to give this user an driver role!",
                        icon: "info",
                        buttons: true,
                        dangerMode: true,
                    })
