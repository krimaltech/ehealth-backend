<div class="chart_container" id="monthly_analytics_chart_{{$chartId}}"></div>
...
{{-- <script>
    ...
    google.charts.setOnLoadCallback(monthlydrawChart_{{ $chartId }});

    function monthlydrawChart_{{ $chartId }}() {
        ...
        var options = {
            title: '{{ $chartTitle }} (Monthly Stats)',
            colors: ['#3366CC'],
            pointSize: 5,
            legend: {
                position: 'bottom'
        ...
        var chart = new google.visualization.LineChart(document.getElementById('monthly_analytics_chart_{{ $chartId }}'));

        chart.draw(data, options);
    }
</script> --}}
<script>
    $('#monthly-{{$id}}-tab').on('shown.bs.tab', function (e,$chartId) {
        // Get the chart container element
        var chartContainer = document.getElementById('monthly_analytics_chart_{{ $chartId }}');
